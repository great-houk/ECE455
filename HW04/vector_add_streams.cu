...
int main() {
	...
	cudaMemcpyAsync(d_A + half, A + half, half_size, cudaMemcpyHostToDevice,
					stream2);
	cudaMemcpyAsync(d_B + half, B + half, half_size, cudaMemcpyHostToDevice,
					stream2);
	...
	vector_add<<<blocks_half, threads, 0, stream2>>>(d_A + half, d_B + half,
													 d_C + half, half);

	cudaMemcpyAsync(C, d_C, half_size, cudaMemcpyDeviceToHost, stream1);
	cudaMemcpyAsync(C + half, d_C + half, half_size, cudaMemcpyDeviceToHost,
					stream2);
